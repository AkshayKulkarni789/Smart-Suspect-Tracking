...
class PrettyWidget(QtWidgets.QWidget):
    ...
    def initUI(self):
        ...
        Model_dir = "F:/sem6/tarp/real world anomaly detection/code/AnomalyDetectionCVPR2018-master/"
    ...
    def SingleBrowse(self):
        video_path = QtWidgets.QFileDialog.getOpenFileName(self,
                                                        'Single File',
                                                        "F:/sem6/tarp/real world anomaly detection/dataset/UCF_Crimes/Videos/")
...
def main():
    ...
    try:
        ...
        if connection.is_connected():
            query="""SELECT Name FROM anomaly"""
            ...
            for r in result:
                if(r[0]==name):
                    query="""UPDATE anomaly set Location ="Bangalore",Time=now() where Name=\""""+name+"""\""""
                    cursor.execute(query)
                    connection.commit()
                    flag=1
                    break;
            if(flag==1):
                break
            query="""INSERT INTO anomaly VALUE (\""""+name+"""\","Bangalore",now())"""
            cursor.execute(query)
            connection.commit()
            cursor.close()
    except Error as e:
